...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  //malloc array inside myBoard
  myBoard->board = malloc(myBoard->height * sizeof(int *));
  ...
  for (int y = 0; y < myBoard->height; y++) {
    myBoard->board[y] = malloc(myBoard->width * sizeof(int *));
    if (myBoard->board[y] == NULL) {
      printf("malloc returned error\n");
      exit(EXIT_FAILURE);
    }
  }

  //set all squares to UNKNOWN
  for (int y = 0; y < myBoard->height; y++) {
  ...
  //add mines
  for (int i = 0; i < myBoard->totalMines; i++) {
    addRandomMine(myBoard);
  }
  //return a pointer to a variable in the Heap
  return myBoard;
}
...
int countMines(board_t * b, int x, int y) {
  ...
  for (int j = y - 1; j < y + 2; j++) {
    for (int i = x - 1; i < x + 2; i++) {
      if (j >= 0 && j <= b->height - 1 && i >= 0 && i <= b->width - 1) {
        numOfMines = numOfMines + IS_MINE(b->board[j][i]);
      }
    }
  }
  numOfMines = numOfMines - IS_MINE(b->board[y][x]);
