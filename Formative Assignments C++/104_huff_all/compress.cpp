...
void writeCompressedOutput(const char * inFile,
  ...
  while (inputFile.get(c)) {
    assert(theMap.find((unsigned char)c) != theMap.end());
    bfw.writeBitString(theMap.find((unsigned char)c)->second);
  }
  assert(theMap.find(256) != theMap.end());
  BitString EOFencoding = theMap.find(256)->second;
  bfw.writeBitString(EOFencoding);
